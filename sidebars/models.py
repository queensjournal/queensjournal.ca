...
class ArtsCalendarItem(models.Model):
    ...
    class Admin:
        fields = (
            ('Event Info/Location', {'fields': ('name', 'location', 'event_type', 'description')}),
...
class ArtsCalendarShowtime(models.Model):
    ...
    item = models.ForeignKey(ArtsCalendarItem) #, edit_inline=models.TABULAR, min_num_in_admin=1, num_in_admin=5, num_extra_on_change=3
...
class TalkingHeadsAnswer(models.Model):
    ...
    photo = models.ForeignKey(Photo, help_text="Photos should be 100 pixels square. JPEGs only, please.")
    question = models.ForeignKey(TalkingHeadsItem) #, edit_inline=models.TABULAR, min_num_in_admin=1, num_in_admin=5, num_extra_on_change=2
