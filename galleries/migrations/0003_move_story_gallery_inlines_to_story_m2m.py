...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        stories_with_galleries = orm['stories.Story'].objects.filter(
            gallery__isnull=False)
        for story in stories_with_galleries:
            for gallery in story.gallery_set.all():
                story.galleries.add(gallery)
                story.gallery_set.remove(gallery)

    def backwards(self, orm):
        stories_with_galleries = orm['stories.Story'].objects.filter(
            galleries__isnull=False)
        for story in stories_with_galleries:
            for gallery in story.galleries.all():
                story.gallery_set.add(gallery)
                story.galleries.remove(gallery)
    ...
    models = {
        ...
        u'galleries.gallery': {
            ...
            'slug': ('django.db.models.fields.SlugField', [], {'max_length': '50'}),
            'story_old': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['stories.Story']", 'null': 'True', 'blank': 'True'})
        },
        u'issues.issue': {
        ...
        u'stories.photo': {
            ...
            'creation_date': ('django.db.models.fields.DateField', [], {'default': 'datetime.datetime(2014, 3, 6, 0, 0)'}),
        ...
        u'stories.story': {
            ...
            'galleries': ('django.db.models.fields.related.ManyToManyField', [], {'related_name': "'story'", 'symmetrical': 'False', 'to': u"orm['galleries.Gallery']"}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
