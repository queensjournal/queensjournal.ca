...
def profile_edit(request, blog):
    if not request.user.is_authenticated():
        ...
        if request.POST:
            ...
            form = ProfileForm({'first_name': profile.user.first_name,
    ...
    if not request.user.is_authenticated():
        ...
        if request.GET.get("s", ""):
            ...
            for term in query.split(" "):
                qs = qs.filter(Q(title__icontains=term) | Q(content__icontains=term) | Q(categories__name__icontains=term) | Q(author__user__first_name__icontains=term) | Q(author__user__last_name__icontains=term))
            c['query'] = query
        # filter by status
