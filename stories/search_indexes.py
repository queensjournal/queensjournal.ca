...
class StoryIndex(SearchIndex):
	...
	def prepare_author(self, obj):
		try:
			sauthor = StoryAuthor.objects.filter(story__pub_date=self.prepared_data['pub_date'])[0]
	...
	def prepare_tags(self, obj):
		tags = Tag.objects.get_for_object(Story.objects.filter(pub_date=self.prepared_data['pub_date'])[0])
		if not tags is None:
			return tags
		else:
			return u'No Tags'
